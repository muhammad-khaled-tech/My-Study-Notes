...
> [!bug] 🕵️ فخ الانترفيو
> **السؤال:** "إيه الفرق بين `pip install django` و `pip3 install django`؟"
>
> **الإجابة الصح:** على أنظمة فيها Python 2 و Python 3 مع بعض، `pip` ممكن يشير لـ Python 2 و `pip3` يشير لـ Python 3. لما بتكون جوا virtual environment، `pip` و `pip3` الاتنين بيشيروا لنفس Python إللي اتعمل بيه الـ venv. الأفضل دايماً تشتغل جوا venv.
>
> **الفخ التاني:** لو نسيت تشغّل الـ venv وعملت `pip install django`، ده هيتثبت على الـ system Python، مش على المشروع. كل مشروع المفروض يكون عنده venv منفصل.
