...
# 🌐 Backend & Web من الصفر (Q1 > [!info] 📖 إزاي تذاكر الملف ده؟
> الملف ده مبني بترتيب هرمي: كل موضوع أساس للي بعده، بادئين من "إزاي الإنترنت شغال" ووصولاً لقرارات تصميم أنظمة كاملة زي اللي بتاخدها في منصة تجارة إلكترونية متكاملة. المستوى بيتدرج من Junior (تعريفات وأساسيات) لحد Senior (Trade-offs وقرارات معمارية حقيقية)، فلو لقيت نفسك واقف في سؤال معين، ده مؤشر كويس على المستوى اللي وصلتله دلوقتي.
...
تخيل إنك دلوقتي بتكتب `https://mail.google.com` في المتصفح عشان تفتح الـ Gmail بتاعك — نفس منطق التكامل مع الـ APIs الخارجية (زي Payment Gateway) اللي مشروعك بيتكلم معاها. إنت بتدوس Enter وفي أقل من ثانية، الصفحة بتظهر قدامك. السؤال اللي أي Backend Developer لازم يقدر يجاوب عليه بثقة في الإنترفيو هو: "إيه اللي حصل بالظبط في الثانية دي؟". المتصفح مش بيعمل سحر — هو بيمشي في سلسلة خطوات منطقية، كل خطوة فيها بتحل مشكلة معينة، وكل خطوة من دول هي في الحقيقة موضوع كامل هنشرحه بالتفصيل في الأسئلة الجاية. فكّر في السؤال ده كـ "الخريطة الكبيرة" اللي كل حاجة تانية في الملف ده هتتحط فيها.
...
```mermaid
flowchart TD
    A["المستخدم بيكتب mail.google.com ويدوس Enter"] --> B["DNS Lookup → إيجاد الـ IP Address"]
...
لما بتفتح موقع في المتصفح، الخطوات اللي فوق بتحصل وبعدين المتصفح بياخد HTML وبيعرضه. لما مشروعك (منصة تجارة إلكترونية) بيعمل استدعاء لـ Payment Gateway API (زي Stripe) عشان يتعامل مع المدفوعات، نفس بالظبط الخطوات دي بتحصل (DNS → TCP → TLS → HTTP Request/Response)، لكن الفرق إن الـ Response مش HTML للعرض، هو JSON بيتحول لـ Objects جوا الكود بتاعك.
...
```mermaid
flowchart TD
    A["المتصفح: فين IP بتاع mail.google.com؟"] --> B["1. يدور في الـ Browser Cache"]
    B -->|لو مش موجود| C["2. يدور في الـ OS Cache"]
    C -->|لو مش موجود| D["3. يسأل الـ DNS Resolver بتاع ISP"]
    D -->|لو مش موجود عنده| E["4. يسأل الـ Root Name Server"]
    E --> F["5. يسأل الـ TLD Server بتاع .com"]
    F --> G["6. يسأل الـ Authoritative Name Server بتاع google.com"]
    G --> H["يرجّع الـ IP Address، ويتخزن في كل الـ Caches للمرة الجاية"]
```
...
في مشروع زي منصة تجارة إلكترونية، لو شغّال محلياً Node.js API (NestJS أو Express) وقاعدة بيانات PostgreSQL وRedis كلهم على نفس الجهاز، الجهاز عنده IP واحد بس (127.0.0.1 أو localhost)، لكن كل خدمة شغالة على Port مختلف (الباك اند على 3000، Postgres على 5432، Redis على 6379). لو حصل تعارض Port (يعني خدمتين حاولوا ياخدوا نفس الرقم)، واحدة منهم مش هتشتغل — ده أشهر مشكلة بتقابل أي مطور وهو بيظبط بيئة التطوير المحلية.
...
في منصة تجارة إلكترونية، لما الـ Frontend (أو أي Customer بيستخدم الموقع) بيطلب يشوف تفاصيل طلب (Order) معين، هو الـ Client. الـ Backend بتاعك اللي بيستقبل الطلب، يتحقق من الصلاحيات، يجيب البيانات من PostgreSQL، ويرجّعها — ده الـ Server. لكن لاحظ حاجة مهمة: نفس الـ Backend بتاعك ده لما بيتكلم مع Payment Gateway API (زي Stripe) عشان ينفذ عملية دفع، هو بقى Client من منظور Stripe، وStripe هي الـ Server! يعني نفس السيرفر ممكن يكون Client في علاقة تانية.
...
تخيل مشروعك بعد ما يتنشر، عندك آلاف الـ Customers بيفتحوا منصة التجارة الإلكترونية بتاعتك في نفس الوقت. كل واحد منهم Client منفصل بيبعت Requests مستقلة، والسيرفر بتاعك (Backend) لازم يكون قادر يستقبل ويعالج كل الطلبات دي بالتوازي من غير ما طلب واحد يعطل التاني — ده أول تلميح لموضوع الـ Concurrency والـ Scalability اللي هنتعمق فيه في الموضوع السابع.
...
| Network/Link Layer | النقل الفعلي على الوسط المادي (كابل/واي فاي) | Ethernet, Wi-Fi |نسيان تحديد الـ Port في التطوير المحلي
...
في مشروعك، لما بتبعت Access Token أو بيانات بطاقة حساسة لـ Payment Gateway API، لو ده بيحصل عبر HTTP عادي، أي حد على نفس الشبكة يقدر "يشم" (Sniff) الترافيك ده ويسرق الـ Token. بالـ HTTPS، حتى لو حد نجح يعترض البيانات، هيلاقيها مشفرة ومش قادر يفهمها من غير المفتاح.
...
لو فريقك الستة أشخاص كل واحد شغّال محلياً NestJS على Port 3000 وPostgreSQL على 5432، ولو حد منهم شغال Redis كمان لمشروع تاني على نفس الجهاز، ممكن يحصل تعارض. عشان كده في مشاريع المنصات والتطبيقات الحديثة، استخدام Docker Compose (هنشرحه بالتفصيل في الموضوع الثامن) بيحل المشكلة دي لأنه بيعزل كل خدمة في بيئتها الخاصة، وتقدر تتحكم في الـ Port Mapping بشكل صريح ومتفق عليه في فريقك كله.
...
```http
POST /api/v1/orders/1001/items HTTP/1.1
...
URL     → /api/v1/orders/1001/items                (على أنهي مورد؟)
Headers → Host, Content-Type, Authorization        (معلومات إضافية عن الطلب)
Body    → { "productId": "p123", "quantity": 2 }   (البيانات الفعلية المرفقة)
```
...
curl -X GET "https://api.ecommerce.com/api/v1/orders/1001" \
...
curl -X POST "https://api.example.com/orders" -d '{"productId": "p123"}'
...
#### مثال 3: حالة عملية — طلب معقد في مشروعك (Order Pagination)

عندما تطلب صفحة تانية من قائمة الطلبات (Orders) من API منصة التجارة الإلكترونية، الطلب هيبقى فيه Query Parameters جوا الـ URL نفسه (زي `page=2` أو `cursor=xyz`) عشان تحدد "من أنهي نقطة أكمل"، وHeaders فيها الـ Authorization Token، وممكن Body لو الطلب POST. فهمك الدقيق لبنية الطلب هنا مباشرة بيأثر على قدرتك تصمم آلية الـ Pagination صح وتتعامل مع الأخطاء النصفية بشكل احترافي.
...

{
  ...
  "status": "processing",
  "items": [
    { "id": "p1", "name": "Laptop Stand", "quantity": 1 }
  ]
}
```
...
Body        → { "orderId": "1001", ... } (البيانات الفعلية المطلوبة)
```
...
في سيناريو تصفح الطلبات (Order Pagination)، الـ Response المثالي مش بس بيرجّع البيانات، لازم يرجّع معلومات إضافية في الـ Body (أو حتى Headers) توضح "فيه صفحة تانية ولا لأ" و"إيه الـ Cursor اللي أستخدمه للصفحة الجاية". وبما إن الأنظمة المعقدة بتتطلب التعامل مع البيانات الكبيرة، الـ Response في الحالة دي المفروض يوضح بوضوح معلومات الـ Metadata عشان الـ Client يتعامل معاها صح.
...
| Method | الغرض | مثال |
|---|---|---|
| GET | جلب بيانات موجودة | `GET /orders/1001` |
| POST | إنشاء مورد جديد | `POST /orders` |
| PUT | استبدال مورد بالكامل | `PUT /orders/1001` |
| PATCH | تعديل جزء من مورد | `PATCH /orders/1001` |
| DELETE | حذف مورد | `DELETE /orders/1001` |

الفرق الدقيق بين PUT وPATCH هو أشهر فخ في السؤال ده: PUT معناه "استبدل المورد ده بالكامل بالبيانات اللي بعتهالك" — يعني لو نسيت تبعت Field معين، هيتحط Null أو يتمسح. بين```
...
بعد ما بنينا API منظّم ومتسق في الأسئلة اللي فاتت، فيه خطر عملي حقيقي: لو Client (أو Script غلط، أو حتى هجوم متعمد) بعت آلاف الـ Requests في ثانية واحدة لـ `/orders`، السيرفر ممكن يقع أو يبطّئ لكل المستخدمين التانيين. الحل هو الـ Rate Limiting: تحديد أقصى عدد Requests مسموح بيها لكل Client في فترة زمنية معينة.�خزنها تلقائياً)، وكل مرة تروح فيها لنفس المكان، بتوريهم البطاقة من غير ما حد يسألك "مين إنت؟" من الأول.
...
sequenceDiagram
    ...
    C->>S: Login (user/pass)
    S->>R: Create session, store {userId: 5, role: "admin"}
    S-->>C: Set-Cookie: sessionId=xyz
    C->>S: GET /profile (Cookie: sessionId=xyz)
    S->>R: Lookup session xyz
    R-->>S: {userId: 5, role: "admin"}
    S-->>C: Response with profile data
```
